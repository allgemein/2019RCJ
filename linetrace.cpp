...
delay(300);
  ...
  while (analogRead(phtC) >= limen) {
    ...
    if(analogRead(phtC) < limen) break;
    delay(1);
...
void pid() {
  ...
  //出力値を表示
  sprintf(str, "p:%4d i:%4d d:%4d diff:%4d",propotial,integral,differential,diff);
...
void searchLine() {
  ...
  if (moveUntilLineFound(F, 250)) return;

  while (1) {

    //黒い線を見つけるか100ms経過するまで右へ旋回する。黒い線を見つけたら関数の処理を終了
    if (moveUntilLineFound(R, 500)) return;
    //黒い線を見つけるか200ms経過するまで左へ旋回する。黒い線を見つけたら関数の処理を終了
    if (moveUntilLineFound(L, 1000)) return;
    //黒い線を見つけるか100ms経過するまで右へ旋回する。黒い線を見つけたら関数の処理を終了
    if (moveUntilLineFound(R, 500)) return;
    //黒い線を見つけるか100ms経過するまで前進する。黒い線を見つけたら関数の処理を終了
    if (moveUntilLineFound(F, 250)) return;
